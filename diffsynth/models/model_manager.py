...
def load_model_from_single_file(state_dict, model_names, control_layers, model_classes, model_resource, torch_dtype, device):
...
class ModelDetectorFromSingleFile:
    ...
    def load(self, file_path="", state_dict={}, control_layers=15, device="cuda", torch_dtype=torch.float16, **kwargs):
        if len(state_dict) == 0:
            state_dict = load_state_dict(file_path)
        ...
        if keys_hash_with_shape in self.keys_hash_with_shape_dict:
            ...
            loaded_model_names, loaded_models = load_model_from_single_file(state_dict, model_names, control_layers, model_classes, model_resource, torch_dtype, device) # dit init
            return loaded_model_names, loaded_models
...
class ModelManager:
    ...
    def load_model(self, file_path, control_layers=15, model_names=None, device=None, torch_dtype=None):
        ...
        for model_detector in self.model_detector:
            if model_detector.match(file_path, state_dict):
                model_names, models = model_detector.load( # dit init
                    file_path, state_dict, control_layers,
    ...
    def load_models(self, file_path_list, control_layers=None, model_names=None, device=None, torch_dtype=None):
        for file_path in file_path_list:
            self.load_model(file_path, control_layers, model_names, device=device, torch_dtype=torch_dtype) # dit init
